% 目标函数
function P=fitness(hangbanxulie)
...
for i=1:1733
     if(x(i,1)~=0&& x(i,2)~=0)
          if(hangbanxulie(1,x(i,1))~=0&&hangbanxulie(1,x(i,2))~=0)%港口号其中有一个等于零的就不再考虑
               count_renshu=count_renshu+lvke_biaojian{i,5};
...
for i=1:303
    if(hangbanxulie(1,i)~=0)
        count=count+1;
        fangzheng(i,hangbanxulie(1,i))=1;
    end
end
count_xjuzhen=0;
 for h=1:69
...
P=-(0.3*(count/qulianggang1)+0.6*(pinjun_huanchengjinzhangdu/(qiuhe_huanchengjinzhangdu/count_renshu))+0.1*(count_xjuzhen/qulianggang3));